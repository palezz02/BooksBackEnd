...
public class BookImpl implements IBookService{
	...
	public BookImpl(IBookRepository bookRepository, IPublisherRepository publisherRepository,
			IAuthorRepository authorRepository, ICategoryRepository categoryRepository, IInventoryRepository invenntoryRepository) {
	...
	public void update(BookReq req) throws BooksException {
		...
		if (req.getLanguageCode() != null) book.setLanguageCode(req.getLanguageCode());
		if (req.getPublicationDate() != null) book.setPublicationDate(req.getPublicationDate());
